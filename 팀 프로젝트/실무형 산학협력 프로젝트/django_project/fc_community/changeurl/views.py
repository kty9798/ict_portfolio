...
def keyword(request):
    ...
    if(rescode==200):
        ...
        for i in range(len(category_list)):
            ...
            category_result += result_i
        
        count_ca = Counter(category_result)
        tag_count_ca = []
        tags_ca = []

        for n, c in count_ca.most_common(100):
            dics = {'tag': n, 'count': c}
            #if len(dics['tag']) >= 2 and len(tags) <= 49:
        ...
        print(tag_count_ca)
        final_result_ca = ""
        for i in range(len(tag_count_ca)): #숫자로 임의 지정하면 안뜨는데 len(tag_count_ca) 로 하니까 되네.......
        ...
        for i in soup.findAll(class_ ="relatedTags_relation_srh__1CleC"):
            for j in i.findAll("a"):
                relate_contents += j.get_text()
                relate_contents+= ",  "
                if number==7:  #나오는 숫자 개수
                    break    
                else:
                    number+=1
        ...
        print("연관 검색어는:" ,relate_contents)     

        #print(title_list)
        
        return render(request , "marketing.html" , { 'keyword' : keywords, 'recommend_keyword': final_result , 'recommend_category' : final_result_ca , 'relate_content' : relate_contents})   #결과
